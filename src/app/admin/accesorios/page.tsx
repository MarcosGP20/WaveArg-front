...
import { AccesoriosService, Accesorio } from "@/lib/api";
...
export default function InventarioPage() {
  ...
  const cargarAccesorios = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await AccesoriosService.getAll();
      setAccesorios(data);
    } catch (err: unknown) {
  ...
  const handleDelete = async (id: number) => {
    setIsDeleting(true);
    try {
      await AccesoriosService.delete(id);
      toast.success("Accesorio eliminado correctamente");
      setDeletingId(null);
      await cargarAccesorios();
    } catch (err: unknown) {
      toast.error(err instanceof Error ? err.message : String(err) ?? "Error al eliminar el Accesorio");
    } finally {
      setIsDeleting(false);
    }
  };
  ...
  const AccesoriosAgrupados = Accesorios.reduce((acc: Accesorio[], current) => {
      ...
      <div className="bg-white rounded-xl shadow-md overflow-hidden border border-gray-100">
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="divide-y divide-gray-100">
            {/* AQUÍ CAMBIAMOS Accesorios por AccesoriosAgrupados */}
            {AccesoriosAgrupados.map((p) => {
                      ...
                      {p.variantes && p.variantes.length > 0 ? (
                        ...
                        <div className="flex items-center gap-4">
                          <p className="text-sm font-medium text-orange-700">
                            ...
                            Eliminá primero las variantes para poder borrar el Accesorio.
                          </p>
                          <Link
                            href={`/admin/accesorios/${p.id}/variantes`}
