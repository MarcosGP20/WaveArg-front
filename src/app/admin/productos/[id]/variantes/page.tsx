...
export default function GestionVariantesPage() {
  ...
  // Función para refrescar la data después de guardar o borrar
  ...
  const onSubmit = async (data: any) => {
    ...
    toast.promise(savePromise(), {
      ...
      success: () => {
        ...
        cargarInformacion(); // Recarga la tabla de abajo automáticamente
  ...
  // Función de borrado fuera de onSubmit para que sea accesible
  const handleDeleteVariante = async (varianteId: number) => {
    if (!window.confirm("¿Estás seguro de eliminar esta variante de stock?"))
      return;

    const deletePromise = VariantesService.delete(varianteId);
    ...
    toast.promise(deletePromise, {
      ...
      success: () => {
        setProducto((prev) => {
          ...
          return {
            ...prev,
            variantes: prev.variantes.filter((v) => v.id !== varianteId),
          };
        });
    ...
    <div className="p-6 max-w-7xl mx-auto">
      <div className="flex items-center gap-4 mb-8">
        <Link
          ...
          className="text-gray-500 hover:text-[#05467d]"
        >
          <ArrowLeft size={24} />
        </Link>
        <div>
          <h1 className="text-2xl font-bold text-[#05467d]">
            {loadingProducto
              ? "Cargando..."
              : `Gestionar Variantes de "${producto?.nombre} ${producto?.modelo}"`}
          </h1>
          {!loadingProducto && (
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <button
          ...
          className="w-full bg-[#05467d] text-white py-4 rounded-xl font-bold flex justify-center items-center gap-2 shadow-lg hover:bg-[#043a68] transition-all"
        >
          {isSubmitting ? (
