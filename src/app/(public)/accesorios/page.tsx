...
function AccesoriosContent() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        setLoading(true);
        const data = await AccesoriosService.getAll(true); // Assuming API takes soloDisponibles similarly
        setProducts(data);
      } catch (err) {
...
export default function AccesoriosPage() {
  return (
    <Suspense fallback={<div className="p-20 text-center font-bold">Cargando accesorios...</div>}>
      <AccesoriosContent />
    </Suspense>
  );
}
